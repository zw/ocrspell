/*
 * $Id: cnf.c,v 1.1.1.1 2000/08/28 21:11:37 slumos Exp $
 *
...
/*
 * Routines to generate confusions from the longest common 
 * subsequence.  An attempt is made to select the optimal 
 * confusions.
 */
...
static char rcsid[] = "$Id: cnf.c,v 1.1.1.1 2000/08/28 21:11:37 slumos Exp $";
...
static struct 
c_v *compare_confusion_vectors(struct c_v *, struct c_v *, int, int);

static void 
vector_confusion_values(struct c_v *, int *, int *, int *, int, int);

static struct confusion *confusion_generation(char *, char *, int *);

static struct 
confusion *new_confusion(char *, char *, int *, int, int, int, int);
...
int **Vectors;
/*
 * Generate confusions for the two strings using a longest common
 ...
 */
{
     ...
     if (v > 1) {
	  ...
	  for (left=confusion_vector_list, i=1; i<v; 
	       left=best, i++) {
	       best = compare_confusion_vectors
		    (left, confusion_vector_list+i, m, n);
	  }
...
int m, n;
/*
 * Determine the best confusion set
 */
{
     int *vpp;
...
int *V;
/*
 * Generate all of the confusions for future use as device
 * mappings
 */
{
     ...
     for (i=0, groups=0, cnfs=0, in_group=0, expected=1; i<n; ) {
	  ...
	  if (i < n || (expected <= m || V[n-1] != m)) {
		...
		*/
	       ...
	       if (!(new_cnf = 
		     new_confusion(string1, string2, &difference,
...
/*
 * Organize and group all new confusions
 */
{
